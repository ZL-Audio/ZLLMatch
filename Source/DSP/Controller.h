...
class Controller {
public:
    explicit Controller (juce::AudioProcessor* processor, juce::AudioProcessorValueTreeState& parameters) {
        ...
        if (modeID == ZLDsp::mode::learn) {
            ...
            if (learning) {
                if (mainMonitor.getBufferPeak() < gate) {
                    if (sideID == 1) {
                        matcher.learnDiff (mainMonitor.getLoudness(), targetMonitor.getLoudness(), mainMonitor.getPeak());
                    } else {
                        matcher.learnDiff (mainMonitor.getLoudness(), target, mainMonitor.getPeak());
                    }
                    mainMonitor.reset();
                    targetMonitor.reset();
                    learning = false;
                ...
                if (mainMonitor.getLoudness() > gate) {
                    learning = true;
                }
            }
    ...
    void setModeID (int ID) {
        if (modeID == ZLDsp::mode::learn && ID == ZLDsp::mode::effect && periodID == 0) {
...
private:
    ...
    std::atomic<FloatType> strength = ZLDsp::strength::formatV (ZLDsp::strength::defaultV);
...
template <typename FloatType>
class ControllerAttach : public juce::AudioProcessorValueTreeState::Listener, private juce::Timer {
public:
    ...
    void timerCallback() override {
        if (modeID == ZLDsp::mode::effect) {
            ...
            apvts->getParameter (ZLDsp::gain::ID)->setValueNotifyingHost (ZLDsp::gain::range.convertTo0to1 (controller->getGain()));
    ...
    void parameterChanged (const juce::String& parameterID, float newValue) override {
        if (parameterID == ZLDsp::loudness::ID) {
            ...
            controller->setStrength (ZLDsp::strength::formatV (static_cast<FloatType> (newValue)));
